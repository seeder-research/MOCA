function [] = froof_line()
    ...
    for m=1:var1_bound
        for n=1:var2_bound
            ...
            max_energy = 1e10000;   % initialize as infinity
    ...
    for i=1:8
        ...
        % set(ax,'zscale','log');
        zlim([4, 8]);
        ax.XAxis.TickValues = [10, 30, 50, 70];
        ax.YAxis.TickValues = [10, 30, 50, 70];
        % ax.ZAxis.TickValues = [1.0e0, 1.0e2, 1.0e4, 1.0e6];

        exportgraphics(gcf,path+"/fig-roofline-"+string(i)+".jpg",'ContentType','image', 'Resolution', 300);
    end
...
%     grid on
    ...
    for j=1:len_point_list(2)
        i = point_list(j);
        % plot( log10 (1./record_min_time(i,:) ), "-*", 'linewidth', 3);
        loglog(log_x, 1./record_min_time(i,:), ":*", 'linewidth', 3, 'DisplayName',''+string(i)+'0');
        hold on;
    end
    grid on;
    ax = gca;
    ...
    title(leg,'BW_{mm}')
    exportgraphics(gcf,path+"/fig-roofline-roofline.jpg",'ContentType','image', 'Resolution', 600);

    figure;
    [X,Y] = meshgrid(1:var1_bound,1:var2_bound);
    surfc(X, Y, log10( record_min_time ));
    exportgraphics(gcf,path+"/fig-roofline-surf.jpg",'ContentType','image', 'Resolution', 300);
    ...
    save(path+'/record_type_min_time.mat','record_type_min_time');
    save(path+'/record_min_time.mat','record_min_time');
end
